...
# creating route53 (insecure_devops pic , it is amazon provided dns not our domain dns hence it shows insecure --> in order to avoid such scenerios we use route53)
resource "aws_route53_record" "frontend_alb" {
  zone_id = var.zone_id
  name    = "roboshop-${var.environment}.${var.domain_name}" # roboshop-dev.akshaysunny.space
  type    = "A"
